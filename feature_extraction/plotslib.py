...
def strf2avgvec(strf):
    ...
    avgvec = np.concatenate(
        (np.ravel(strf_scale_rate), np.ravel(
            strf_freq_rate), np.ravel(strf_freq_scale))
    )
    return avgvec


def avgvec2strfavg(avgvec, nbChannels=128, nbRates=22, nbScales=11):
    # idxFSR = nbChannels * nbRates * nbScales
    idxSR = nbRates * nbScales
    idxFR = nbChannels * nbRates
    idxFS = nbChannels * nbScales
    # strf_fsr = np.reshape(avgvec[:idxFSR], (nbChannels, nbScales, nbRates))
    strf_scale_rate = np.reshape(avgvec[:idxSR], (nbScales, nbRates))
    strf_freq_rate = np.reshape(
        avgvec[idxSR: idxSR + idxFR], (nbChannels, nbRates))
    strf_freq_scale = np.reshape(
        avgvec[idxSR + idxFR:], (nbChannels, nbScales))
...
def plotStrfavg(
    ...
    plt.subplot(1, 3, 1)
    plt.imshow(
        strf_scale_rate, aspect=aspect_, interpolation=interpolation_, origin="lower"
    )
    ...
    plt.subplot(1, 3, 2)
    plt.imshow(
        strf_freq_rate, aspect=aspect_, interpolation=interpolation_, origin="lower"
    )
    plt.xticks([])
    plt.yticks([])
    plt.xlabel("Rates (Hz)", fontsize=10)
    ...
    plt.subplot(1, 3, 3)
    plt.imshow(
        strf_freq_scale, aspect=aspect_, interpolation=interpolation_, origin="lower"
    )
    plt.xticks([])
    plt.yticks([])
...
def plotStrfavgEqual(
    ...
    im = plt.imshow(
        strf_scale_rate,
        aspect=strf_scale_rate.shape[1] / strf_scale_rate.shape[0],
    ...
    im = plt.imshow(
        strf_freq_rate,
        aspect=strf_freq_rate.shape[1] / strf_freq_rate.shape[0],
        interpolation=interpolation_,
        origin="lower",
        cmap=cmap,
    )
    ...
    im = plt.imshow(
        strf_freq_scale,
        aspect=strf_freq_scale.shape[1] / strf_freq_scale.shape[0],
        interpolation=interpolation_,
        origin="lower",
        cmap=cmap,
    )
